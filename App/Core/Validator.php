...
class Validator
{
    ...
    private function checkStr($value)
    {
        ...
        if ($this->value_min == $this->value_max && mb_strlen($value) != $this->value_min) {
            return Validator::formatText(ERROR_MESSAGES["string_length"], ["{number}"], [$this->value_min]);
        }
        if (mb_strlen($value) < $this->value_min) {
            return Validator::formatText(ERROR_MESSAGES["string_min_length"], ["{number}"], [$this->value_min]);
        }
        if (mb_strlen($value) > $this->value_max) {
            return Validator::formatText(ERROR_MESSAGES["string_max_length"], ["{number}"], [$this->value_max]);
        }
        // Check Special Rule
        return ($this->special_checker)($value);
    }
